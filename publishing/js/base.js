...
function waterStageFrom(device) {
  if (!device?.online || device.water === null || device.water === undefined) return "unk";
...
function waterText(stage) {
  switch (stage) {
    case "crit": return "심각 (90%↑)";
    case "warn": return "경계 (80%↑)";
    case "caution": return "주의 (70%↑)";
    case "ok": return "정상 (<70%)";
...
function stagePillClass(stage) {
  switch (stage) {
    case "crit": return "pill pill--crit";
    case "warn": return "pill pill--warn";
    case "caution": return "pill pill--caution";
    case "ok": return "pill pill--ok";
    default: return "pill pill--unk";
  }
}
...
function markerDotClass(stage) {
  switch (stage) {
    case "crit": return "nm-dot nm-dot--crit";
    case "warn": return "nm-dot nm-dot--warn";
    case "caution": return "nm-dot nm-dot--caution";
    case "ok": return "nm-dot nm-dot--ok";
    default: return "nm-dot nm-dot--unk";
  }
}
...
function renderMarkers() {
  ...
  state.devices.forEach(d => {
    ...
    btn.type = "button";
    btn.className = "map-marker" + (d.id === state.activeId ? " is-active" : "");
    ...
    btn.addEventListener("click", () => selectDevice(d.id, { openPopup: true }));
...
function renderList() {
  ...
  shown.forEach(d => {
    ...
    card.addEventListener("click", () => selectDevice(d.id, { openPopup: true }));
  ...
  setInterval(() => {
    ...
    if (!elPopup.hidden && state.activeId) {
      ...
      if (active) {
        ...
        elLv70.textContent = active.water != null && active.water >= 70 ? "해당" : "해당 없음";
        elLv80.textContent = active.water != null && active.water >= 80 ? "해당" : "해당 없음";
        elLv90.textContent = active.water != null && active.water >= 90 ? "해당" : "해당 없음";

        updatePopupNetwork(active);
        applyWaterBadges({ device: active, pillEl: null, popupEl: elPopupWaterBadge });
